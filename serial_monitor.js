...
export class SerialMonitor {
  constructor(state) {
    ...
    this.outputFilename = 'serial_log.txt';
    this.port = null;
    this.reader = null;
    this.baud = 9600;
    ...
    this.recordedData = []; // Store data for CSV export
    this.init();
  }
  ...
  renderSidebar() {
    ...
    sb.innerHTML = `
      ...
      <div class="sidebar-section">
        ...
        <button id="cmd-calibrate" class="sidebtn quick-cmd">Calibrate</button>
        <button id="cmd-test" class="sidebtn quick-cmd">Test Connection</button>
      </div>
    ...
    // Quick command handlers - now properly implemented
    document.getElementById('cmd-start').onclick = () => this.sendQuickCommand('start_logging', 'Starting data logging...');
    document.getElementById('cmd-stop').onclick = () => this.sendQuickCommand('stop_logging', 'Stopping data logging...');
    document.getElementById('cmd-reset').onclick = () => this.sendQuickCommand('reset', 'Resetting device...');
    document.getElementById('cmd-status').onclick = () => this.sendQuickCommand('status', 'Requesting device status...');
    document.getElementById('cmd-calibrate').onclick = () => this.sendQuickCommand('calibrate', 'Starting calibration...');
    document.getElementById('cmd-test').onclick = () => this.sendQuickCommand('test', 'Testing connection...');
    
    this.renderTags();
    setStatusBar(`${this.connected?'Connected':'Disconnected'} | Port: COM4 | ${this.baud} baud | Serial Monitor`);
  }

  sendQuickCommand(command, description) {
    // Show what we're doing
    this.appendTerminal(`[info] ${description}`);
    ...
    setTimeout(() => {
      switch(command) {
        case 'start_logging':
          this.appendTerminal('[info] Data logging started successfully');
          break;
        case 'stop_logging':
          this.appendTerminal('[info] Data logging stopped');
          break;
        case 'reset':
          this.appendTerminal('[info] Device reset complete');
          this.appendTerminal('[info] System ready');
          break;
        case 'status':
          this.appendTerminal('[info] Device Status: Online');
          this.appendTerminal('[info] Sensors: 13 active');
          this.appendTerminal('[info] Sample Rate: 24.0 Hz');
          this.appendTerminal('[info] Memory: 78% available');
          break;
        case 'calibrate':
          this.appendTerminal('[info] Calibration in progress...');
          setTimeout(() => {
            this.appendTerminal('[info] Calibration complete');
          }, 2000);
          break;
        case 'test':
          this.appendTerminal('[info] Connection test: PASS');
          this.appendTerminal('[info] Signal strength: Good');
          break;
      }
    }, 500);
  }
  ...
  setOutputFile() {
    showModal(`
      ...
      <p>Choose the filename for recording data. The file will be saved in CSV format for easy analysis.</p>
      <input type="text" id="output-filename" placeholder="serial_data.csv" value="${this.outputFilename}" style="width: 100%; margin: 1em 0;">
      <div class="file-format-info">
        <small>Supported formats: .csv (recommended), .txt</small>
      </div>
      ...
      document.getElementById('save-output-file').onclick = () => {
        ...
        if (!filename) filename = 'serial_data.csv';
        
        // Ensure CSV extension if no extension provided
        if (!filename.includes('.')) {
          filename += '.csv';
        }
        
        window.parentSerialMonitor.outputFilename = filename;
        window.parentSerialMonitor.renderSidebar();
  ...
  appendTerminal(line) {
    ...
    // If recording and line contains CSV data, store it
    if (this.recording && this.isCSVData(line)) {
  ...
  isCSVData(line) {
    // Check if line contains comma-separated numeric data
    const trimmed = line.trim();
    if (!trimmed || trimmed.startsWith('[') || trimmed.startsWith('SENT:') || trimmed.startsWith('ERROR:')) {
      return false;
    }
    ...
    return numericCount >= parts.length * 0.7; // At least 70% numeric
  }
  ...
  toggleRecording() {
    if (this.recording) {
      ...
      if (this.recordedData.length > 0) {
        ...
        // Fallback to terminal buffer
        this.exportTerminalBuffer();
      }
      ...
      this.recording = true;
      this.recordedData = [];
      this.appendTerminal('[info] Recording started - saving to ' + this.outputFilename);
    }
    this.renderSidebar();
  }
  ...
  exportRecordedData() {
    let content;
    const isCSV = this.outputFilename.toLowerCase().endsWith('.csv');
    
    if (isCSV && this.recordedData.length > 0) {
      // Create proper CSV with headers
      const headers = [];
      const firstDataLine = this.recordedData[0];
      const columnCount = firstDataLine.split(',').length;
      
      // Generate column headers
      for (let i = 0; i < columnCount; i++) {
        headers.push(`Channel_${i + 1}`);
      }
      
      content = headers.join(',') + '\n' + this.recordedData.join('\n');
    } else {
      // Plain text format
      content = this.recordedData.join('\n');
    }
    ...
    this.appendTerminal(`[info] Data exported to ${this.outputFilename} (${this.recordedData.length} data lines)`);
  }

  exportTerminalBuffer() {
    // Fallback: export terminal buffer
    const content = this.buffer.join('\n');
    const mimeType = this.outputFilename.toLowerCase().endsWith('.csv') ? 'text/csv' : 'text/plain';
    const blob = new Blob([content], { type: mimeType });
    const a = document.createElement('a');
    a.href = URL.createObjectURL(blob);
    a.download = this.outputFilename;
    ...
    this.appendTerminal(`[info] Terminal buffer exported to ${this.outputFilename}`);
  }
